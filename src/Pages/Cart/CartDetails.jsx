    ...
    return (
        ...
        <div className="max-w-screen-xl px-4 mx-auto 2xl:px-0">
          ...
          {cartDetails?.items?.length > 0 ? (
                        ...
                        <div className="flex-1 w-full min-w-0 md:order-2 md:max-w-md">
                          <p className="text-base font-medium text-gray-900 hover:underline">
                            <Link to={`/products/${item?.product?._id}`}>
                              {`Product Name:${item?.product?.productName}`}<br/> {`Description : ${item?.product?.description}`} <br/> {`Category: ${item?.product?.category}`}
                            </Link>
                          </p>
            ...
            <div className="m-10 bg-gradient-to-r from-amber-50 to-orange-300  justify-center items-center "><p className="text-2xl">Your Cart Is Empty</p>
            <button className="border border-red-400 bg-gradient-to-r from-amber-50 to-orange-300 rounded-3xl min-w-500 min-h-20 text-3xl "><Link to={'/'}>Continue Browsing</Link></button>
            </div>
          )}
        </div>
        </section>
